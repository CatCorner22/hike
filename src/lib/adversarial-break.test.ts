...
describe("adversarial-break: land-nav honesty", () => {
  ...
  it("fails closed on invalid return and check-in times", () => {
    // An unparseable deadline must not read as overdue (that is a false SOS
    // prompt) but must never be silently ignored either. The honest encoding is
    // valid: false with a label stating the alarm is not armed, and callers
    // render on `overdue || !valid`.
    const invalid = overdueStatus("not-a-date");
    expect(invalid.valid).toBe(false);
    expect(invalid.overdue).toBe(false);
    expect(invalid.label).toMatch(/invalid/i);
    expect(invalid.label).toMatch(/not armed/i);
