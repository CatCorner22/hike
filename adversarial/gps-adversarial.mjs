...
try{const context=await browser.newContext({serviceWorkers:'allow',permissions:['geolocation'],geolocation:{latitude:37.7749,longitude:-119.5383,accuracy:5}});const page=await context.newPage();page.on('pageerror',e=>log('pageerror',false,e.message,'HIGH'));await page.goto(BASE,{waitUntil:'domcontentloaded'});const id='plan-gps-adversarial';await putPack(page,id);await page.goto(`${BASE}/navigate/${id}`,{waitUntil:'domcontentloaded',timeout:60000});await page.waitForSelector('canvas',{timeout:30000});await sleep(1500);let body=await page.locator('body').innerText();log('gps/live-initial-load',/Offline pack|Saved to device/.test(body)&&!body.includes('This page couldn'),body.slice(0,120).replace(/\s+/g,' '));
