...
| [CalTopo](https://training.caltopo.com/all_users/mobile/offline) | Planning-centric map layers, offline mobile maps, recorded tracks, collaborative/team workflows, and a deliberate offline mode | Klandagi has a focused leave-behind trip card but not CalTopo's mature map-printing, layer, and team operations | Keep expert tools behind disclosure; expand printing only where it strengthens handoff or interoperability before adding more calculators |
...
|---|---|---|
| [MapLibre GL JS](https://github.com/maplibre/maplibre-gl-js) | GPU-accelerated browser vector maps; already used online | Keep. The dependency is not the verified route-only offline fallback. |
...
- Made activity recording queue-first and identity-stable across offline/online transitions; Stop drains in-flight fixes, and a hard-reload recovery remains paused until the hiker explicitly resumes.
...
- Added a printable/downloadable leave-behind card and facts-first terrain difficulty evidence that distinguishes measurements, community reports, and unknowns.
- Required separate allow-listed evidence for every model-generated research field; a general citation can no longer legitimize unrelated difficulty, hazard, parking, permit, dog, or camping claims.
...
- Kept PMTiles terrain behind source/license, attribution, byte-integrity, eviction, and physical-device gates; AirNow AQI behind credentials and source-state handling; accounts/sync behind identity, privacy, conflict, and multi-device validation; and ML, 3D, native, and watch work behind their documented adoption or device gates.
